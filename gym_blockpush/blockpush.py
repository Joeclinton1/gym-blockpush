...
from gym_blockpush.utils import utils_pybullet, xarm_sim_robot
...
class BlockPush(gym.Env):
    ...
    def _setup_pybullet_scene(self):
        ...
        target_urdf_path = INSERT_URDF_PATH if self._task == BlockTaskVariant.INSERT else ZONE_URDF_PATH

        self._target_id = utils_pybullet.load_urdf(self._pybullet_client, target_urdf_path, useFixedBase=True)
        self._block_ids = [
            utils_pybullet.load_urdf(self._pybullet_client, BLOCK_URDF_PATH, useFixedBase=False)
        ]
...
class BlockPushNormalized(gym.Env):
    ...
    def step(self, action):
        ...
        state, reward, done, truncated, info = self._env.step(self.calc_unnormalized_action(action))
