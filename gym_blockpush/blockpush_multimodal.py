...
from gym_blockpush import blockpush
from gym_blockpush.utils import utils_pybullet
from gym_blockpush.utils.pose3d import Pose3d
from gym_blockpush.utils.utils_pybullet import ObjState, XarmState
...
class BlockPushMultimodal(blockpush.BlockPush):
    ...
    def __init__(
        ...
        abs_action=False,
        observation_height=blockpush.IMAGE_HEIGHT,
        observation_width=blockpush.IMAGE_WIDTH,
    ):
        ...
        Args:
          ...
          obs_type: Optional "state" or "pixels". if pixels image observations will be used.
    ...
    def _setup_pybullet_scene(self):
        ...
        self._target_ids = [
            ...
            for i in [blockpush.ZONE_URDF_PATH, ZONE2_URDF_PATH]
        ]
        self._block_ids = []
        for i in [blockpush.BLOCK_URDF_PATH, BLOCK2_URDF_PATH]:
    ...
    def _reset_block_poses(self, workspace_center_x):
        ...
        def _reset_block_pose(idx, add=0.0, avoid=None):
            def _get_random_translation():
                block_x = (
                    workspace_center_x + add + self._rng.uniform(low=-RANDOM_X_SHIFT, high=RANDOM_X_SHIFT)
                )
    ...
    def reset(self, seed=None, options=None, reset_poses=True):
        # Seed the random number generator
        super(blockpush.BlockPush, self).reset(seed=seed)
        self.seed(seed)
        ...
        if reset_poses:
            ...
            translation = np.array([0.3, -0.4, blockpush.EFFECTOR_HEIGHT])
    ...
    def _compute_state(self):
        ...
        for i in range(2):
            ...
            elif self._init_distance[i] != 100 and np.abs(new_distance - self._init_distance[i]) > 1e-3:
    ...
    def get_obs(self, state):
        if self.obs_type == "state":
            return np.concatenate(list(state.values()), axis=-1)
        elif self.obs_type == "environment_state_agent_pos":
            # Split environment-related parts (blocks and targets) and agent-related parts (effector), and flatten
            return {
                "environment_state": np.concatenate(
                    [
                        state["block_translation"],
                        state["block_orientation"],
                        state["block2_translation"],
                        state["block2_orientation"],
                        state["target_translation"],
                        state["target_orientation"],
                        state["target2_translation"],
                        state["target2_orientation"],
                    ],
                    axis=-1,
                ),
                "agent_pos": np.concatenate(
                    [state["effector_translation"], state["effector_target_translation"]], axis=-1
                ),
            }
    ...
    def _step_robot_and_sim(self, action):
        ...
        target_effector_pose = Pose3d(
            rotation=blockpush.EFFECTOR_DOWN_ROTATION, translation=target_effector_translation
        )
        ...
        for _ in range(self._sim_steps_per_step):
            if self._connection_mode == pybullet.SHARED_MEMORY:
                blockpush.sleep_spin(frame_sleep_time)
    ...
    def _create_observation_space(self, image_size):
        ...
        # Define environment-related components with appropriate bounds
        env_obs = spaces.Box(
            low=np.array([-5, -5, -pi2, -5, -5, -pi2, -5, -5, -pi2, -5, -5, -pi2]),
            # translation (x, y) and orientation (phi)
            high=np.array([5, 5, pi2, 5, 5, pi2, 5, 5, pi2, 5, 5, pi2]),
            shape=(
                12,
            ),  # 12 dimensions: 2 (x, y) + 1 (orientation) for each block and target (2 blocks, 2 targets)
            dtype=float,
        )

        # Define agent-related observation space using workspace bounds
        agent_obs = spaces.Box(
            # Repeat lower bounds [lowx, lowy] for both translations
            low=np.tile(blockpush.WORKSPACE_BOUNDS[0] - 0.1, 2),
            # Repeat upper bounds [highx, highy] for both translations
            high=np.tile(blockpush.WORKSPACE_BOUNDS[1] + 0.1, 2),
            shape=(4,),  # 4 dimensions: [effector_translation (x, y), effector_target_translation (x, y)]
            dtype=float,
        )

        # Concatenate the environment and agent spaces with agent_obs inserted after index 5 in env_obs
        # This is the ordering of the unmodified state and the raw dataset
        combined_obs = spaces.Box(
            low=np.concatenate((env_obs.low[:6], agent_obs.low, env_obs.low[6:])),
            high=np.concatenate((env_obs.high[:6], agent_obs.high, env_obs.high[6:])),
            shape=(env_obs.shape[0] + agent_obs.shape[0],),  # Update the shape accordingly
            dtype=float,
        )

        # Define pixel observation if image_size is provided
        if self.obs_type in ["pixels", "pixels_agent_pos"]:
            pixel_obs = spaces.Box(
                low=0,
                high=255,
                shape=(image_size[0] * image_size[1] * 3,),  # flattened RGB image
                dtype=np.uint8,
            )
        ...
        # Handle observation types
        if self.obs_type == "state":
            return combined_obs

        elif self.obs_type == "environment_state_agent_pos":
            obs_dict["environment_state"] = env_obs
            obs_dict["agent_pos"] = agent_obs

        elif self.obs_type == "pixels":
            obs_dict["pixels"] = pixel_obs
    ...
    def set_pybullet_state(self, state):
        ...
        for target_state, target_id in zip(state["targets"], self._target_ids, strict=False):
            _set_state_safe(target_state, target_id)

        obj_ids = self.get_obj_ids()
        assert len(state["objects"]) == len(obj_ids), "State length mismatch"
        for obj_state, obj_id in zip(state["objects"], obj_ids, strict=False):
