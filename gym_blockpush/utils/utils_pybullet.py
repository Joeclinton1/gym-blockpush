...
def load_urdf(pybullet_client, file_path, *args, **kwargs):
    ...
    try:
        ...
        raise FileNotFoundError("Cannot load the URDF file {}".format(file_path)) from e
...
class ObjState:
    ...
    def set_bullet_state(self, client, obj_id):
        ...
        for i, (joint_info, joint_state) in enumerate(zip(self.joint_info, self.joint_state, strict=False)):
            ...
            if cur_joint_info != joint_info:
                raise ValueError(
                    "joint_info mismatch {} vs {} (expected)".format(str(cur_joint_info), str(joint_info))
                )
...
def _serialize_pybullet_state(pybullet_state):
    ...
    if isinstance(pybullet_state, list):
        ...
        raise ValueError(
            "Unhandled type for object {}, type {}".format(str(pybullet_state), type(pybullet_state))
        )
